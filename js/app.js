...
(function ($) {
  ...
  /* ------------------------------------------------------------------
     ...
     ------------------------------------------------------------------
     Components live in js/components.js as window.NEXORA.* so they can be
     injected with jQuery's .html() — this works over BOTH http:// and
     file:// (no AJAX, so no CORS blocking when opening files directly).
     ------------------------------------------------------------------ */
  var loadComponents = function () {
    if (window.NEXORA) {
      $("#header").html(NEXORA.header);
      $("#navbar").html(NEXORA.navbar);
      $("#footer").html(NEXORA.footer);
      bindNavbar();
      highlightActiveNav();
      setYear();
    }
  ...
  var initJobFilter = function () {
    ...
    var applyFilters = function () {
      ...
      grid.find(".job-item").each(function () {
        ...
        if (sal !== "all" && $job.data("salary") !== sal) match = false;
    ...
    $("form[data-validate]").each(function () {
      ...
      form.on("submit", function (e) {
        ...
        setTimeout(function () {
          ...
          form[0].reset();
          var success = form.siblings(".form-success").length
            ? form.siblings(".form-success")
            : form.closest(".form-wrap").find(".form-success");
          if (!success.length) {
            success = $('<div class="form-success alert alert-success mt-3" role="alert">Thank you! Your message has been sent. Our team will reach out shortly.</div>');
            form.after(success);
          }
          success.show();
          setTimeout(function () { success.fadeOut(800); }, 6000);
        }, 900);
      });
    });
  };
